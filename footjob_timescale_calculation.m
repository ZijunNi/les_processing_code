%%%%%%%%%%%% 数据读取部分 %%%%%%%%%%%%%%
 ...
 %%
    clear,clc
    % ----------- 自有程序数据
    filename = 'fluid_lagrange_2025-10-18_09-13-34.mat';
...
% 自有数据时间尺度计算：单个粒子的计算
    ...
    % 计算两种方法的时间尺度
    t_l_fit = calculate_timescale(all_data, all_time, 'fit');
    t_l_integral = calculate_timescale(all_data, all_time, 'integral');
    
    % 保存结果
    all_loc(num_sample,1:3) = loc;
    all_t_l_fit(num_sample) = t_l_fit;
    all_t_l_integral(num_sample) = t_l_integral;
    ...
    if(1)
        ...
        [x_inte,inte,zero_cross_timescale,zero_cross_point] = integrate_from_start(lags,rho);
        [~,t_l_fit_temp,r_squared,x_fit,y_fit] = lesutils.fit_exp_model(lags, rho, 0);
        t_l_fit_temp = -1/t_l_fit_temp;
        ...
        plot(lags, rho,DisplayName='Autocorrelation Function');
        hold on
        plot(x_fit,y_fit,'r-.',DisplayName=['Exponential Fit $\rho(s)=\exp(s/T_L)$' newline ' with $T_L = $',num2str(t_l_fit_temp)]);
        grid on;
        legend('Interpreter','latex')
        title('Autocorrelation Funtion')
        ...
        plot(x_inte,inte);
        hold on
        line([0.5 x_inte(end)],[t_l_fit_temp,t_l_fit_temp],'linestyle','--');
        line([0.5 x_inte(end)],[zero_cross_timescale,zero_cross_timescale],'linestyle','--');
        text(1,t_l_fit_temp+0.005,['$T_L = $', num2str(t_l_fit_temp)],'Interpreter','latex')
        text(1,zero_cross_timescale+0.005,['$T_L = $', num2str(zero_cross_timescale)],'Interpreter','latex')
        grid on;
        title('Integration of Autocorrelation Funtion')
    end
    toc
    fprintf('已完成：%.2f %% \n\n',num_sample/length(particleData)*100)
% end

% save(['timescale_',filename],"all_t_l_fit","all_t_l_integral","all_loc")

%% 绘图部分
y_plus_loc = all_loc(:,2)*180;
scatter(y_plus_loc,all_t_l_integral,DisplayName='$T_L$ Values');
% set(gca,'XScale','log')
hold on
...
if(contains(filename,'euler'))
    ...
    for i = 1:length(uniq_loc)
        ...
        % 提取对应B中的值
        valuesFromB = all_t_l_integral(indices);
        % 计算平均值
...
elseif(contains(filename,'lagrange'))
    ...
    for i = 1:length(uniq_loc)-1
        ...
        if ~isempty(indices)
            % 提取对应的C值
            c_values = all_t_l_integral(indices);
