...
y_points = 1-y_plus*delta_vis;
z_points = linspace(0.0, 3 * pi, nz);
...
% 示例数据
data = result(:,2); % 正态分布数据，均值5，标准差2
% clear centers counts
% 更精确的箭头标注版本
figure;

% 创建直方图和密度曲线
% h = histogram(data, 'Normalization', 'pdf', 'FaceAlpha', 0.2);
% hold on;
% 计算概率密度函数曲线
[counts, edges] = histcounts(data, 'Normalization', 'pdf');
centers = edges(1:end-1)+diff(edges);
plot(centers, counts, 'r-', 'LineWidth', 2);

% 计算极值点
min_val = min(data);
max_val = max(data);
[~, min_idx] = min(abs(xi - min_val));
[~, max_idx] = min(abs(xi - max_val));
...
% 转换数据坐标到图形坐标
% [min_x_norm, min_y_norm] = data_to_normalized(ax, min_val, f(min_idx));
% [max_x_norm, max_y_norm] = data_to_normalized(ax, max_val, f(max_idx));

% 添加文本标注
text(min_val, f(min_idx) + 1e+6, sprintf('Minimum: %.2e', min_val), ...
    'HorizontalAlignment', 'center', 'BackgroundColor', 'white', ...
    'FontSize', 12, 'FontWeight', 'bold');

text(max_val, f(max_idx) + 1e+6, sprintf('Maximum: %.2e', max_val), ...
    'HorizontalAlignment', 'center', 'BackgroundColor', 'white', ...
    'FontSize', 12, 'FontWeight', 'bold');

% 添加箭头（使用annotation，需要归一化坐标）
% annotation('arrow', [min_x_norm min_x_norm], [min_y_norm+0.1 min_y_norm+0.02], ...
%     'Color', 'red', 'LineWidth', 2, 'HeadWidth', 10, 'HeadLength', 10);
% 
% annotation('arrow', [max_x_norm max_x_norm], [max_y_norm+0.1 max_y_norm+0.02], ...
%     'Color', 'red', 'LineWidth', 2, 'HeadWidth', 10, 'HeadLength', 10);

title(['Wall-normal Velocity $v$ at $y^+ = $',num2str(y_plus)],Interpreter="latex");
xlabel('$v$','Interpreter','latex');
ylabel('PDF');
grid on;

% 辅助函数：将数据坐标转换为归一化坐标
% function [x_norm, y_norm] = data_to_normalized(ax, x_data, y_data)
%     x_lim = xlim(ax);
%     y_lim = ylim(ax);
%     x_norm = (x_data - x_lim(1)) / (x_lim(2) - x_lim(1));
%     y_norm = (y_data - y_lim(1)) / (y_lim(2) - y_lim(1));
% end
...
x_points = linspace(0, 8 * pi, nx);
y_points = linspace(-1, -1+5/1000, ny);
z_points = linspace(0.2 * pi, 0.4 * pi, nz);
...
save("JHTDB_Re_tau_1000.mat","results","x_points","y_points","z_points");
toc
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:24
    ...
    time_end = 1 + i;
    % Time step.
    delta_t = (time_end-time_start)/1000;
    ...
    filename = [num2str(time_start),'_to_',num2str(time_end),'.mat'];
    
    save(filename,"times_plot","result")
